...
def rfidSeen(tag,rfidName,home,TimeOut,TimeHome,TimeHomeTotal,rfidPins,TimeIn,TimeAway,TimeAwayTotal,rfidtags,rfidNick):
    ...
    printPerson(tag,rfidName, home,TimeHome,TimeHomeTotal,TimeAway,TimeAwayTotal,rfidNick)
    return;
        
def printPerson(tag,rfidName, home,TimeHome,TimeHomeTotal,TimeAway,TimeAwayTotal,rfidNick):
    lcd.clear()
    for i in range (0,4):
        if home[i]:
            status='H'
    ...
    print('Time Home Total: ' + str(TimeHomeTotal[tag]))
    print('Time Away Total: ' + str(TimeAwayTotal[tag]))
    print('Time Home: ' + str(TimeHome[tag]))
    print('Time Away: ' + str(TimeAway[tag]))
    return;
